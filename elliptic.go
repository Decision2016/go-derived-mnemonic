...
type CurveParams struct {
	...
	N *big.Int // the linear coefficient of the curve equation
}
...
func (curve *CurveParams) doubleJacobian(x, y, z *big.Int) (*big.Int, *big.Int, *big.Int) {
	...
	if big.NewInt(-3).Cmp(curve.N) == 0 {
		...
		alpha.Add(alpha, x2)
		if new(big.Int).Cmp(curve.N) != 0 {
			delta.Mul(delta, delta)
			delta.Mul(curve.N, delta)
